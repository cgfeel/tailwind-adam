...
-   当只存在 `generateStaticParams` 预构生成的静态资源，才能将整个项目导出为静态 `html`
...
-   注意第一段话末尾：`Data Cache`，这是 `NextJS` 的一种缓存模式，不是通常说的数据缓存
...
| 缓存数据             | 位置        | 目的                     | 有效期       |
| -------------------- | ----------- | ------------------------ | ------------ |
| `fetch` 函数返回的值 | 服务端-内存 | `React` 组件之间数据共享 | 每次请求周期 |
...
**额外补充：`NextJS` 除了通过 `fetch` 获取请求还有哪些方法？**

| 分类          | 用途                               |
| ------------- | ---------------------------------- |
| `fetch`       | 网络请求                           |
| `DB`          | 从数据库获取数据，如：`Prisma`     |
| `grpc-client` | 可以实现，但目前没有比较成熟的方案 |
| `redis`       | 通过 `redis` 和服务器做通信        |

以上这些方法都不会默认缓存数据结果到 `Data Cache`，需要自行设置：

> 除此之外，只有 `fetch` 支持相同请求中，跨组件数据共享
